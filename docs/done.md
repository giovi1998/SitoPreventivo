...
- **Fix sessione 2026-08-21/22 — website preview, CRM, RAG, brief parity, tracing** (branch `feat/ai-skill-harness`, commit fef7e5f): (1) **Preview logo** — `imageNormalize` protegge `data-qb-logo` dal placeholder GIF (50KB→placeholder bug live: logo sparito in preview + element picker dava placeholder all'AI); `logoInjection` marca `data-qb-logo="1"`; preview 256→512px (sotto ~400px il modello non legge gerarchie). (2) **Mappa** — `enforceMapIframe` converte anche iframe OpenStreetMap→Google Maps (l'AI generava OSM invece di Google). (3) **Hamburger + JS fallback** — `buildWebsiteFullDocument` (funnel unico preview/export/vision) parsa con `new Function` e wrappa in try/catch con flag `__qbSiteJsFailed` → fallback `MENU_FALLBACK_JS` a runtime (ReferenceError + SyntaxError); sandbox `allow-popups allow-popups-to-escape-sandbox`; `ensureMenuToggleButton` inietta bottone se l'AI lo omette; `ensureHamburgerCss` + rigenera con fallback title per doc legacy senza brief; prompt form contatti reso OPZIONALE (non finto action="#"). Test: `websiteExport` 19, `imageNormalize` + `sanitizeGenerated` (OSM, hamburger, applyPaletteVars, ensureMenuToggle), `e2e/website-mobile-menu.spec.ts` 4 casi (JS valido/rotto/vuoto su viewport 375px). (4) **CRM logo** — `handleRemoveLogo` azzera `logoUrl`+`detectedLogoUrl` con log onesto, Zod nullable, PATCH include chiave. (5) **RAG spam** — `isJunkChunk`/`cleanMarkdownForKnowledge` (≥2 U+FFFD, >50% URL) a livello paragrafo prima del chunking in `crm.ts`+`firecrawlLocal.js`/`crm.js` locale, research idempotente (DELETE prima di insert), mock DB esteso; test knowledgeTopK + spam regression. (6) **Brief parity** — `customers.pages/sections/cta/features` (migration `magenta_silver_sable`), `Create/UpdateCustomerSchema`, `handler` POST/PATCH, `syncCustomerToWebsiteDocs/Local`, `buildBriefContext`/`Api`, `CustomerDetail` 4 campi editabili, website draft usa campi cliente. (7) **Tracing** — `onLog` skill attive/tool/verify/coherence/resume in `useAutoBuildGenerate` + `activeSkillLabels`, step intermedi `generateWebsiteDraft` + `CustomerDetail` handleGenerateOne, test `useAutoBuildGenerate` 42 + `skillLibrary` + `CustomerDetail`. (8) **Golden examples curati** — fix `iconGlyph`→`iconName`/`monogram`, `qualityScore`/`scoreReason`, `Cookie` allowlist, email coerente. (9) **Coherence t21** — `applyPaletteVars` CSS `:root` override, `DocumentAiStats` già incluso; `verifyRender` 512px + brief reale. Script `scripts/chiccheria-compare.mjs` salvato per confronto A/B onesto stesso brief prod vs branch (logo escluso, card/flyer/website promozionali). Gate: 298 file 3467 test verdi, typecheck OK. Gotchas §28 esteso.

- **Bidirezionale CRM↔card/website completata (semi-completa)** (commit b166e8f): esteso `website→customer` a `pages/sections/cta/features` (`src/components/WebsiteEditor.tsx:196`); nuovo `customer→card` (`src/server/crm.ts:67` `syncCustomerToCardDocs` + `src/utils/dataService/crm.js:62` `syncCustomerToCardLocal`, last-write-wins) e `card→customer` (`src/hooks/useCardAutoSave.ts:45` `syncCardToCustomer` su save/autosave con `skipSync:true` — email/phone/website/address/socials/company/name/title/font/accent). Ogni editor ora aggiorna CRM; CRM aggiorna tutti i doc collegati se più vecchi. Test: `CustomerDetail.test.tsx:44` + `useAutoBuildGenerate` 42, `skillLibrary` 16, `sanitizeGenerated` 33, `websiteExport` 19; typecheck OK. Gotchas §28 esteso con sezione bidirezionale.

## 2026-08-20

- **Harness AI: skill di progetto richiamabili dall'AI di tutti gli editor + tool `load_skill` nell'agente** (richiesta utente "migliora l'harness, se hai le skill nel progetto dovresti poterle richiamare"): (1) nuovo `src/ai/skillLibrary.ts` — catalogo curato kind→skill (card→`web-design-guidelines`, flyer→`gpt-taste`, logo→`brandkit`, social→`muapi-nano-banana`, website→`high-end-visual-design`, palette→`minimalist-ui`; quote/onboarding: nessuna pertinente), contenuto via import `?raw` dinamici con path letterali (chunk on-demand, §25; `.agents` è committato → file presenti in build Vercel), distill frontmatter YAML + cap 20K caratteri/skill, fallback silenzioso (mai rompere il flusso AI). (2) Injection DOPO `promptRegistry.getPrompt` in tutti gli 8 orchestratori (14 call site `*-system`): compone con gli override remoti Langfuse (TB-029 fase 2) invece di esserne rimpiazzata — regression test card aggiornato (override resta prefisso, skill appesa dopo). (3) `AgentOrchestrator`: tool `load_skill` (enum dal catalogo con descrizioni, contenuto integrale nel messaggio tool via nuovo campo `AgentToolResult.content`, non filtrato da `include`), `MAX_TOOL_ROUNDS` 6→8. (4) Wayfinder: mappa `harness-skill-map.md` + tickets aperti t12 (loop hardening dedup/early-stop), t13 (controllo utente skill), t14 (skill condizionali per stile website), t15 (budget costi). AGENTS.md: fix riga stale "agente non collegato alla UI" (era wirato da t11/2026-08-12) + riga Key Files skillLibrary. Test: skillLibrary 10 nuovi, agent +2, assert TB-029 card aggiornato.


## 2026-08-19

- **Website editor: preview non aggiornata, "Nuova tab" bloccata, font brief non salvato nel cliente, test sito 3 pagine** (da report utente): (1) **Preview non aggiornata su edit testo** — root cause: `CodeEditor.tsx` CodeMirror catturava la PRIMA `onChange` al mount (stale closure) → le modifiche alla pagina secondaria finivano su `website.html` invece di `pagesHtml[page]`; in più `previewPage` e `codePage` erano due stati indipendenti → edit di una pagina, preview di un'altra. Fix: `onChangeRef` sempre fresco nel listener + stato `page` unico condiviso tra preview e codice + `key={page}` sul CodeEditor (remount su cambio pagina). (2) **about:blank#blocked** — `window.open(blobUrl)` bloccato dal popup-blocker. Fix: anchor `<a target="_blank">` sintetico + `URL.revokeObjectURL` dopo 30s. (3) **Font "Edu VIC WA NT Hand Precursive" non salvato nel cliente** — il sync website→customer esisteva SOLO nel save esplicito e con `customerId` settato; l'autosave 30s non toccava mai il customer, e il ramo locale di `updateCustomer` spreadava gli `undefined` clobberando i campi. Fix: helper `syncWebsiteToCustomer` (autosave + save esplicito, solo campi non vuoti) + strip di `undefined`/`''` nei patch locali (`crm.js`). (4) **Test sito 3 pagine** — nuovo `e2e/website-multipage.spec.ts`: preview per pagina, Nuova tab blob, edit testo pagina secondaria → preview aggiornata. 2 E2E verdi.

- **Logo da immagine del cliente (TB-033): imita stile/colori, testo nuovo** (richiesta utente "se il cliente mette un'immagine tipo logo, fallo simile cambiando il testo"): `LogoAIOrchestrator.generateLogo` accetta `referenceImageBase64` (vision) con istruzione esplicita "imita STILE, COLORI e ICONA, mai riprodurre il wordmark/tagline esistenti"; `useAILogo.generate` lo propaga; `LogoAiPanel` ha upload "Logo di riferimento (opzionale)" compresso via `compressForAI` (mai persistito); auto-build CRM (`useAutoBuildGenerate`) comprime `customer.logoUrl/detectedLogoUrl` e lo passa sia al path diretto sia al path agente (`AgentContext.logoReference`). Test: orchestrator 2 nuovi (blocco presente/assente). 3382 unit + 2 E2E verdi.

- **TB-034: coerenza multipagina sito web** (richiesta utente "metti una coerenza tra le pagine, si parlino, stili uguali, niente ripetizioni"): (1) nuovo helper `summarizeIndexHtml()` (`src/ai/prompts/websiteSystem.ts`) che estraggge in modo deterministico dall'index: titolo hero, riga lead, CTA principale e lista sezioni (id); (2) `buildWebsitePagePrompt` riceve ora `indexSummary` e istruisce il modello: CONTENUTO GIÀ PRESENTE IN INDEX (non ripetere slogan/CTA/sezioni), classi condivise obbligatorie (.nav/.brand/.btn/.section-inner/.footer), `.page-hero` ridotto con `.eyebrow` per le pagine interne, breadcrumb/titolo pagina; (3) system prompt: regola `Coerenza M-PAGE` (nav/footer/logo identici, nessuna CTA/hero duplicata, CSS unico con classi condivise); (4) prompt CSS esteso con .page-hero/.eyebrow/.value/.team/.form-group/.form-field; (5) prompt JS: guard sulle sezioni pagine interne (.form/.team) — una sola logica condivisa, mai un handler per pagina. Test: `websiteMultipageCoherence.test.ts` +7 (summary estrazione, dedupe prompt, classi condivise, system prompt). E2E visivo: `e2e/website-coherence-audit.spec.ts` con screenshot su 3 pagine coerenti (nav identica, hero gradient uguale, page-hero ridotto, CTA solo in index) — screenshot salvati in `artifacts/audit-website/` e verificati a mano.

- **kimi-k3:cloud pay-per-token ora tracciato su Langfuse (prod + locale)** (da report utente: costo Kimi K3 assente su Langfuse e UI): `kimi-k3:cloud` è pay-per-token ($3.0/$15.0 per 1M, extra usage oltre il flat $20/mo) ma il server (`computeCostUsd('ollama', ...)` → sempre 0, `src/server/ai.ts`) e il dev proxy (`vite.config.js`, `costUsd: body.costUsd` mai inviato dal client) lo trattavano come flat → `cost_details` mai emesso su Langfuse, in prod E in locale. Fix: tabella `OLLAMA_PER_TOKEN_PRICE` (chiave = model) in `ai.ts` + passaggio usage nei 2 trace Ollama (chat e stream), helper `ollamaCostUsd` nel dev proxy (rami stream e non-stream). Client era già corretto (`providerPricing.ts` PRICING `ollama-kimi-k3` → badge/aiStats ok). Regression test: `ollamaStream.test.ts` +2 (stream kimi cost_details $18, flat minimax assente) +1 (chat non-stream kimi $18), `viteDevProxy.test.ts` +2 (stream kimi $18, flat assente). Docs: tabella provider gotchas aggiornata (19 provider, kimi-k3 marcato extra usage) + regola 8 "pay-per-token su 3 livelli".

- **Bottone ✕ "Cancella log" collegato in tutti gli editor** (da report utente: "non posso cancellare nel social e nel logo il log con la x"): la ✕ esisteva in `AILogPanel` ma (a) `AIHarnessConsole` non inoltrava `onClearLogs` → social/flyer/card non mostravano la ✕; (b) `LogoAiPanel` renderizzava `AILogPanel` senza `onClearLogs`. Fix: prop `onClearLogs` in `AIHarnessConsole` (inoltrata ad AIConsole), collegata in SocialEditor (`reset`), FlyerEditorShell (`handleAiReset`), CardEditorShell (`handleResetCardChat`), LogoAiPanel (`reset`). WebsiteEditor già la aveva. Regression test AIConsole (2: ✕ chiama callback, ✕ assente senza callback) — 12 verdi.

- **Controllo completo sui post social: prompt libero, caption e imagePrompt editabili** (da domande utente: "posso scrivere un prompt? posso modificare il testo a mano? posso scegliere il modello?"): (1) **prompt libero** — rimosso `hidePrompt` dalla rail social, la textarea AI Console ora alimenta `handleGenerate(text)` → `generatePosts(..., { userPrompt })` → nel prompt con header "Istruzioni aggiuntive dell'utente (rispettale, hanno priorità su default e sorgente)"; (2) **caption manuale** — la caption è ora una `<textarea>` editabile per piattaforma (`setPosts` esposto da useAISocial, updater per post); (3) **imagePrompt editabile** — input "Prompt immagine" per piattaforma, il bottone "Genera immagine" usa il valore corrente (per un visual diverso senza rigenerare la caption); (4) **modello immagine** — selettore "Modello immagine" (Gemini Nano Banana / Nano Banana 2 Lite, pattern FlyerAiPanel/CardAICoverSection) nel form, persistito in `pq_ui:v1` via `setAiImageModelDefault`, passato a `generatePostImage(platform, prompt, imageModel)`. Modello chat: già selezionabile dal badge provider (Ollama/DeepSeek) in alto alla rail. Test: orchestrator (1: userPrompt nel messaggio user), UI (3: prompt → generate, caption edit → setPosts, modello immagine → generatePostImage), e2e social-editor aggiornato. 36 test mirati verdi.

- **Sito web come sorgente per i post social** (richiesta utente "nella pagina social non puoi mettere anche sitoweb come fonte?"): `SocialSource` esteso con `{type:'website'}` (`WebsiteSnapshot` da `brief`: businessName, sector, description, target, cta, features, contacts); selettore "Tipo sorgente" ora card/flyer/**sito web**, lista documenti da `brief.businessName` (fallback title/id), snapshot nel prompt come "Sito web di X. Settore/Descrizione/Target/CTA/Servizi/Contatti"; `buildFallbackImagePrompt` per website usa businessName/sector/features; `SocialPage` carica anche `documentType === 'website'`; `sourceDocumentType` schema (z.enum) esteso a `'website'` + `SocialSourceType` condiviso; label log `website "<businessName>"`; vision preview skippata per website (nessun DOM dedicato). Empty state + CTA "Crea sito web". Test: 3 nuovi (prompt website ×2, UI select+fallback) — 3367 verdi.
