...
export const handleCustomers: RouteHandler = async (path, method, req, res, body) => {
  ...
  // TB-012 step 2: deploy Netlify della landing bozza (server-side, token
  // NETLIFY_AUTH_TOKEN mai esposto al browser). Crea il site alla prima
  // esecuzione (nome stabile per cliente), poi crea un deploy con il file
  // uploadato. Ritorna l'URL preview (draft) condivisibile col cliente.
  if (path.startsWith('/customers/') && method === 'POST' && path.endsWith('/landing-deploy')) {
    if (!requireAdmin(req, res, body)) return;
    const token = process.env.NETLIFY_AUTH_TOKEN;
    if (!token) return json(req, res, 503, { error: 'Netlify non configurato (NETLIFY_AUTH_TOKEN mancante)' });
    const id = path.split('/')[2];
    const html = typeof body.html === 'string' ? body.html : '';
    const fileName = typeof body.fileName === 'string' ? body.fileName : '';
    if (!html.trim() || !fileName.trim()) return json(req, res, 400, { error: 'html e fileName richiesti' });
    const db = await getDb();
    const [cust] = await db.select().from(customersTable).where(eq(customersTable.id, id));
    if (!cust) return json(req, res, 404, { error: 'Cliente non trovato' });
