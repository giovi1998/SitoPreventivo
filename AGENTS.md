...
- **Icona AI slot policy (CON-IS-001)**: `handleGenerateIcon` applica l'icona generata sempre in `photoUrl` (sostituisce la foto corrente), preservando `logoUrl` caricato dall'utente. Il logo aziendale non viene mai toccato.
- **Vision screenshot gating (CON-MM-002)**: gli hook AI (`useAI`, `useAICard`, `useAIFlyer`, `useAILogo`) catturano lo screenshot preview solo se `getAiVisionEnabled()` AND `providerSupportsVision(resolvedModelId)` (helper in `src/utils/resolveProviderId.ts`) — con provider text-only (es. DeepSeek) la cattura è saltata del tutto, non catturata e scartata.
...
- **⚠️ Preview/export mismatch noto (v2.15, aggiornato v2.16)**: la preview React e l'export SVG/PDF/PNG non sono ancora perfettamente identici. Stato aggiornato dalla verifica TB-023 (`docs/tb023-verification.md` §10):
  1. **Wrapping testo** (ancora aperto): la preview usa CSS `overflow-wrap: break-word`; l'export usa `wrapTextAtWhitespace` (che ora hard-breaka anche i token lunghi per chunk — il gotcha "email/URL non spezzate" è obsoleto). Nome/titolo/company/wordmark/QR label restano a riga singola in export.
  2. ~~**Densità verticale**~~ **RISOLTO**: `effectiveBackGridForRender` è usato sia dalla preview (`CardPreview.tsx`) sia dall'export (`svgRenderer.ts`) — il short-contacts collapse si riflette in entrambi.
  ...
  4. **Font preview ridotti su mobile ≤900px** (`cardBase.css:152-164`): scelta responsive preview-only, l'export non cambia.
  **Fixati in v2.16** (verifica TB-023): QR fgColor (era `#000000` hardcoded → `card.style.textColor`), placement QR ignorato in preview (regola CSS `.card-grid-cell--qr .card-back-qr`), wash fronte mid-stop (0.4→0.25 come preview), bordo accent foto in export, espansione servizi basata sui contenuti socials, logo fallback in cella foto, `logoBackground:'card'` nella cella logo preview. Regression test: `svgRenderer.test.ts` + `CardPreview.test.tsx` describe "v2.16 preview/export parity fixes (TB-023)", e2e `card-preview-export-parity.spec.ts`.
  I mismatch residui (1, 3, 4) vanno documentati qui finché non si risolvono con un unico layout engine condiviso preview/export.
...
| 15, AI Harness Upgrade (TB-023) | ✅ done | Spec: `spec/spec-design-ai-harness-upgrade.md`. Completato: multi-provider (Ollama Pro + DeepSeek), badge provider con menu (apertura verso il basso, fix clipping), tracking costi (`totalCostUsd`/`lastCostUsd` in `useAILogs`, `AILogPanel`, `AIConsole`, `AIProviderBadge`, `CardAIControls`), 5 pattern decorativi SVG in preview+export, generic `placement {x,y,scale}` per qualsiasi elemento grid con controlli nudge+zoom per foto e QR, Icona AI card (modello+sfondo+prompt library), dev-proxy `/api/ai/chat(/stream)` + `/api/ai/image-flash`. v2.15: QR label export parity, short-contacts collapse. **Sotto-moduli finali**: (1) **A/B provider** — implementato poi **rimosso deliberatamente** (commit `15aa0d5`, non necessario in fase di validazione): `resolveProviderId` è oggi un semplice resolver modelId→pref→default, `aiABTestingEnabled` non esiste più; (2) **Vision feedback screenshot preview** — endpoint `/api/ai/design-review` + `useAIDesignReview`/`aiModule.runDesignReview` esistono ma il bottone UI "Analizza preview" è stato **rimosso deliberatamente** (non funzionava nel flusso attuale): il codice resta orfano, vedere `docs/post-tb023-known-issues.md`. **Feedback post-TB-023 fixati**: costi visibili inline in `AILogPanel` (token/costo/immagine) e nei dettagli espansi (model/request), `lastCostUsd` reattivo via calcolo live sui log, flag `hasImage` e `modelId` per operazioni immagine, test `useAILogs`/`AILogPanel`, rimossa occorrenza UTF8 problematica in `promptUtils.ts`. **v2.16 feedback**: screenshot preview allegato ai log e inviato al modello vision-enabled per card/quote/flyer/logo tramite `data-*-preview` + `captureElementAsBase64`; `useAILogs`/`AILogPanel` mostrano `imagePreviewBase64` nel dettaglio espanso; `useAI`/`useAICard`/`useAIFlyer`/`useAILogo` loggano `hasImage`/`imagePreviewBase64` e propagano `costUsd`/`modelId` nello stream finale; rimossa riga "Risposta AI ricevuta (vedi dettaglio sopra)" duplicata in modalità analisi. **Issue aperti**: vedi `docs/post-tb023-known-issues.md` (icona quadrato vuoto, log preview). **Verifica completa 2026-07-22**: `docs/tb023-verification.md` — checklist REQ con esiti, fix v2.16 (preview/export parity, mobile preset, CON-IS-001, CON-MM-002, REQ-TC-006) e gap documentati (AI decoration, DecorationPicker, UX avanzata). |
...
2. ~~**Back cover wash opacity mismatch**~~. ✅ Fixato.
3. ~~**Icona AI 512px pixelata in export HD**~~. ✅ Fixato.
4. ~~**CONTATTI header nascosto da stacking**~~. ✅ Fixato: `position:relative; z-index:2` su `.card-back-header`.
5. **Log image preview persa al refresh**. By design (QuotaExceeded).
6. **Modulo AI unificato**. Feature TB-023 (provider, A/B,
   vision, fallback) sono implementate ma frammentate in silos.
   Servono come modulo trasversale che ogni editor può usare, non toggle
   sparsi in SettingsPage. Vedi `docs/post-tb023-known-issues.md` §4.
...
- Endpoint admin (`GET /users`, `GET /quotes/all`, `PATCH /users/limits`, `GET /users/cost-breakdown`) richiedono `adminEmail=admin@gmail.com` come query param o body field. Se assente → 403. `GET /users/cost-breakdown?days=N` (REQ-TC-006) ritorna aggregati lifetime per utente (`tokensUsed`, `tokensCostUsd`) + `ollamaProFlatMonthly` — nessuno storico per-chiamata/per-provider nel DB, `days` è solo echo del parametro (deviazione documentata inline in `api/index.ts`).
