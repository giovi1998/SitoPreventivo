...
| 15, AI Harness Upgrade (TB-023) | ⚠️ NOT-STARTED | Spec: `spec/spec-design-ai-harness-upgrade.md`. Multi-provider (Ollama Pro $20/mo flat + MiniMax M3 multimodale + Gemini 2.0 Flash), tracking costi reale (`tokens_cost_usd` col), 5 pattern decorativi SVG (wave/blob/splash/full-overlay) selezionabili manualmente + via AI, drag mouse foto card grid-mode (`photoPlacement {x,y,scale}`), icone stilizzate AI per card+flyer, RAG clienti (tabella `client_kb` + pgvector), A/B provider, vision feedback screenshot preview. ~45h, sprint 2-3. |
